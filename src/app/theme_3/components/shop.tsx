"use client";
import AllProduct from "@/app/theme_3/components/all-product"
...
const Shop = () => {
  ...
  const allTags = Array.from(new Set(sampleProducts.flatMap(product => product.tags)));
  ...
  useEffect(() => {
    ...
    if (selectedPriceRange !== 'all') {
      ...
      if (range) {
        filtered = filtered.filter(product =>
          product.discounted_price >= range.min && product.discounted_price <= range.max
        );
      }
    }
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
                ...
                <div className="mb-6">
                  <div className="flex flex-wrap gap-2">
                    {selectedCategories.filter(c => c !== 'all').map(category => (
                      <span key={category} className="inline-flex items-center gap-1 bg-green-50 text-green-700 px-3 py-1 rounded-full text-sm">
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 transition-transform ${expandedFilters.categories ? 'rotate-180' : ''}`} />
                </button>
                    ...
                    {categories.map(category => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedCategories.includes(category.id) ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
                ...
                {expandedFilters.price && (
                  <div className="space-y-2">
                    {priceRanges.map(range => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedPriceRange === range.id ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 transition-transform ${expandedFilters.brands ? 'rotate-180' : ''}`} />
                </button>
                {expandedFilters.brands && (
                  <div className="space-y-2">
                    {brands.map(brand => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedBrands.includes(brand.id) ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
                ...
                {expandedFilters.rating && (
                  <div className="space-y-2">
                    {ratings.map(rating => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedRating === rating.id ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                >
                  <div className={`w-3 h-3 bg-white rounded-full transform transition-transform ${inStockOnly ? 'translate-x-6' : 'translate-x-1'} mt-1`} />
                </button>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex border rounded-lg overflow-hidden">
                    <button
                      ...
                      className={`px-4 py-2 transition-colors ${viewMode === 'grid'
                          ? 'bg-green-600 text-white'
                    ...
                    <button
                      ...
                      className={`px-4 py-2 transition-colors ${viewMode === 'list'
                          ? 'bg-green-600 text-white'
                          : 'bg-white text-gray-600 hover:bg-gray-100'
                        }`}
